...
export default function CreateNodeDialog({ isOpen, type, parentId, onClose }) {
    const [name, setName] = useState('');
    const [isSubmitting, setIsSubmitting] = useState(false);
    const { createFile, createFolder } = useEditorStore();
    ...
    const handleSubmit = async (e) => {
        ...
        let res;
        if (type === 'file') {
            res = await createFile(roomId, name, parentId);
        } else {
            res = await createFolder(roomId, name, parentId);
        }
        if (res) {
            onClose();
            setName('');
