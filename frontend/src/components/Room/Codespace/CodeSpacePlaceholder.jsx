...
const CodeSpace = () => {
  ...
  const { selectedFile, fetchFileContent,saveCode } = useFileStore();
  ...
  const handleSaveCode=async()=>{
    ...
    const res=saveCode(selectedFile.id,sourceCode);
    if(res){
  ...
  useEffect(() => {
    ...
    const fetchFile = async () => {
      ...
      setFileLoading(true);
      console.log("h2", selectedFile.id);
      const res = await fetchFileContent(selectedFile.id);
      if (res) {
      ...
      <div className="flex-1 flex flex-col h-full min-w-0 bg-[#1e1e1e]">
        ...
        <div className="h-12 bg-surface/50 border-b border-muted/20 flex items-center justify-between px-4 backdrop-blur shrink-0">
          ...
          <div className="flex items-center gap-3">
            <span className="text-xs text-muted font-mono bg-white/5 px-2 py-1 rounded">{selectedFile ? selectedFile.name : "click to open file"}</span>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            >
              {isLoading ? <Loader2 size={14} className="animate-spin" /> : <Save size={14} fill="currentColor" />}
              Save
            </button>)
            }
