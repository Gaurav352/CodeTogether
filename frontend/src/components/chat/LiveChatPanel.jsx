...
export default function LiveChatPanel() {
    ...
    // Make sure this correctly grabs your user object from the store!
    ...
    // 1. Initialize Listeners & Fetch History (Only if empty!)
    useEffect(() => {
        if (!roomId) return;
    ...
    const handleSend = async () => {
        ...
        const success = await sendMessage(roomId, formData);
                    ...
                    return (
                        ...
                        >
                            <div className={`flex gap-2 sm:gap-3 max-w-[92%] sm:max-w-[85%] md:max-w-[75%] ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
                                ...
                                {/* Avatar */}
                                <div className={`flex-shrink-0 w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center text-[10px] sm:text-xs font-bold shadow-md ${isMe
