...
export default function WorkspaceSidebar({ activeTab }) {
  ...
  const [searchParams, setSearchParams] = useSearchParams();
    const { handleLeaveRoom, currentRoom } = useWorkspaceStore(); 
  ...
  // The function that runs when the user clicks "Yes/Leave" inside the modal
    ...
    <>
      ...
      >
        ...
        <div className="flex flex-col gap-3 px-3">
          ...
          <button 
            // 1. Changed from handleLeaveRoom to opening the modal
      ...
      {/* 2. Modal mounted outside the sidebar flow but in the same fragment */}
      <LeaveRoomModal
        isOpen={isLeaveModalOpen}
        onClose={() => setIsLeaveModalOpen(false)}
        onConfirm={confirmLeaveRoom}
        isLeaving={isLeaving}
        roomName={currentRoom?.name || "this workspace"}
      />
    </>
  );
}